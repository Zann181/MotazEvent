...
def dashboard_barra(request):
    ...
    stats = {
        ...
        "ingresos_totales_hoy": VentaBarra.objects.filter(fecha__date=hoy).aggregate(
            total=Sum("total")
        )["total"]
        or 0,
    }
    ...
    productos_top = (
        Producto.objects.filter(ventabarra__fecha__date=hoy)
        .annotate(
            ventas_hoy=Count("ventabarra"), total_vendido=Sum("ventabarra__total")
        )
        ...
        try:
            ...
            with transaction.atomic():
                MovimientoStock.objects.create(
                    ...
                    observacion=(
                        f"Ajuste: {observacion}" if observacion else "Ajuste de stock"
                    ),
                    usuario=request.user,
                )
...
def movimientos_stock(request):
    ...
    context = {
        ...
        "filtros": {"producto": producto_id, "tipo": tipo},
    }
...
def buscar_cliente_qr(request):
    ...
    try:
        ...
        if not asistente.ha_ingresado:
            return JsonResponse(
                {"success": False, "message": "El asistente no ha ingresado al evento"}
            )
...
def lista_ventas(request):
    ...
    context = {
        ...
        "filtros": {"fecha": fecha, "producto": producto_id},
    }
...
def ventas_del_dia(request):
    ...
    productos_vendidos = (
        ...
        .annotate(total_vendido=Sum("cantidad"), ingresos=Sum("total"))
...
def reportes_barra(request):
    ...
    stats = {
        "productos_activos": Producto.objects.filter(activo=True).count(),
        "valor_inventario_actual": sum(
            p.precio * p.stock for p in Producto.objects.filter(activo=True)
        ),
        "ventas_totales": VentaBarra.objects.count(),
        "ingresos_totales": VentaBarra.objects.aggregate(total=Sum("total"))["total"]
        or 0,
        "ventas_hoy": VentaBarra.objects.filter(fecha__date=hoy).count(),
        "ingresos_hoy": VentaBarra.objects.filter(fecha__date=hoy).aggregate(
            total=Sum("total")
        )["total"]
        or 0,
    }
...
def reporte_final(request):
    ...
    for producto in Producto.objects.filter(activo=True):
        ...
        primer_movimiento = (
            MovimientoStock.objects.filter(producto=producto, tipo="entrada")
            .order_by("fecha")
            .first()
        )
        ...
        # Total vendido
        total_vendido = (
            VentaBarra.objects.filter(producto=producto).aggregate(
                total=Sum("cantidad")
            )["total"]
            or 0
        )
        ...
        # Ingresos generados
        ingresos = (
            VentaBarra.objects.filter(producto=producto).aggregate(total=Sum("total"))[
                "total"
            ]
            or 0
        )
    ...
    # Totales
    totales = {
        "valor_inventario_inicial": sum(
            p["stock_inicial"] * p["producto"].precio for p in productos_reporte
        ),
        "valor_inventario_actual": sum(
            p["stock_actual"] * p["producto"].precio for p in productos_reporte
        ),
...
def exportar_inventario(request):
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Inventario Zann Event"
    ...
    response["Content-Disposition"] = (
        f'attachment; filename=inventario_dmt69_{datetime.now().strftime("%Y%m%d_%H%M")}.xlsx'
    )
    wb.save(response)
    return response
...
def exportar_ventas(request):
    ...
    response["Content-Disposition"] = (
        f'attachment; filename=ventas_dmt69_{datetime.now().strftime("%Y%m%d_%H%M")}.xlsx'
    )
    wb.save(response)
    return response
...
def exportar_movimientos(request):
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Movimientos Stock Zann Event"
    ...
    response["Content-Disposition"] = (
        f'attachment; filename=movimientos_dmt69_{datetime.now().strftime("%Y%m%d_%H%M")}.xlsx'
    )
    wb.save(response)
    return response
